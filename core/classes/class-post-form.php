...
class Odin_Post_Form extends Odin_Front_End_Form {
	...
	protected function save_custom_fields( $post_id, $submitted_data ) {
		if ( ! empty( $this->custom_fields ) ) {
			foreach ( $this->custom_fields as $key ) {
				...
				$data = apply_filters( 'vulcano_post_form_custom_field_data_' . $this->id, $submitted_data[ $key ] );
	...
	protected function save_terms( $post_id, $submitted_data ) {
		if ( ! empty( $this->terms ) ) {
			foreach ( $this->terms as $taxonomy => $term ) {
				// Apply filter to sanitization.
				$term = apply_filters( 'vulcano_post_form_term_data_' . $this->id, $submitted_data[ $term ] );
	...
	public function save_post( $submitted_data ) {
		if ( ! empty( $submitted_data ) ) {
			$post_data = apply_filters( 'vulcano_post_form_insert_data_' . $this->id, array(
			...
			do_action( 'vulcano_post_form_after_save_' . $this->id, $post_id, $submitted_data );
		}
	}
}
