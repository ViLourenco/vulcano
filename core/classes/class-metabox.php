...
class Vulcano_Metabox {
	...
	public function scripts() {
		...
		if ( in_array( $screen->id, $this->get_post_type() ) ) {
			...
			// Metabox.
			wp_enqueue_script( 'vulcano-admin', get_template_directory_uri() . '/core/assets/js/admin.js', array( 'jquery' ), null, true );
			wp_enqueue_style( 'vulcano-admin', get_template_directory_uri() . '/core/assets/css/admin.css', array(), null, 'all' );
			...
			wp_localize_script(
				...
				'vulcanoAdminParams',
				array(
					'galleryTitle'  => __( 'Add images in gallery', 'vulcano' ),
					'galleryButton' => __( 'Add in gallery', 'vulcano' ),
					'galleryRemove' => __( 'Remove image', 'vulcano' ),
					'uploadTitle'   => __( 'Choose a file', 'vulcano' ),
					'uploadButton'  => __( 'Add file', 'vulcano' ),
				)
			);
		}
	}
	...
	public function metabox( $post ) {
		...
		do_action( 'vulcano_metabox_header_' . $this->id, $post_id );

		echo apply_filters( 'vulcano_metabox_container_before_' . $this->id, '<table class="form-table vulcano-form-table">' );

		foreach ( $this->fields as $field ) {
			echo apply_filters( 'vulcano_metabox_wrap_before_' . $this->id, '<tr valign="top">', $field );

			if ( 'title' == $field['type'] ) {
				$title = sprintf( '<th colspan="2"><strong>%s</strong></th>', $field['label'] );
			} elseif ( 'separator' == $field['type'] ) {
				$title = sprintf( '<td colspan="2"><span id="vulcano-metabox-separator-%s" class="vulcano-metabox-separator"></span></td>', $field['id'] );
			} else {
				$title = sprintf( '<th><label for="%s">%s</label></th>', $field['id'], $field['label'] );
			}

			echo apply_filters( 'vulcano_metabox_field_title_' . $this->id, $title, $field );

			echo apply_filters( 'vulcano_metabox_field_before_' . $this->id, '<td>', $field );
			...
			echo apply_filters( 'vulcano_metabox_field_after_' . $this->id, '</td>', $field );

			echo apply_filters( 'vulcano_metabox_wrap_after_' . $this->id, '</tr>', $field );
		}

		echo apply_filters( 'vulcano_metabox_container_after_' . $this->id, '</table>' );

		do_action( 'vulcano_metabox_footer_' . $this->id, $post_id );
	...
	protected function process_fields( $args, $post_id ) {
		...
		switch ( $type ) {
			...
			case 'editor':
				...
				break;
			case 'color':
				$this->field_input( $id, $current, array_merge( array( 'class' => 'vulcano-color-field' ), $attrs ) );
				break;
			case 'upload':
			...
			default:
				do_action( 'vulcano_metabox_field_' . $this->id, $type, $id, $current, $options, $attrs );
				break;
		}
	}
	...
	protected function field_upload( $id, $current, $attrs ) {
		echo sprintf( '<input type="text" id="%1$s" name="%1$s" value="%2$s" class="regular-text"%4$s /> <input class="button vulcano-upload-button" type="button" value="%3$s" />', $id, esc_url( $current ), __( 'Select file', 'vulcano' ), $this->build_field_attributes( $attrs ) );
	}
	...
	protected function field_image( $id, $current ) {
		...
		$html  = '<div class="vulcano-upload-image">';
	...
	protected function field_image_plupload( $id, $current ) {
		$html = '<div class="vulcano-gallery-container">';
			$html .= '<ul class="vulcano-gallery-images">';
				if ( ! empty( $current ) ) {
			...
			$html .= sprintf( '<input type="hidden" class="vulcano-gallery-field" name="%s" value="%s" />', $id, $current );

			// Adds "adds images in gallery" url.
			$html .= sprintf( '<p class="vulcano-gallery-add hide-if-no-js"><a href="#">%s</a></p>', __( 'Add images in gallery', 'vulcano' ) );
		$html .= '</div>';
	...
	public function save( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			if ( ! in_array( $field['type'], array( 'separator', 'title' ) ) ) {
				...
				$new = apply_filters( 'vulcano_save_metabox_' . $this->id, $value, $name );
