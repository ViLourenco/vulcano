...
class Vulcano_Term_Meta {
	...
	public function edit_view() {
		...
		foreach ( $this->fields as $field ) {
			echo '<tr>';
			echo sprintf( '<th><label for="%s">%s</label></th>', $field['id'], $field['label'] );
			echo apply_filters( 'vulcano_term_meta_field_edit_screen_before_' . $this->id, '<td>', $field );
			$this->process_fields( $field );
			if ( isset( $field['description'] ) ) {
				echo sprintf( '<p class="description">%s</p>', $field['description'] );
			}
			echo apply_filters( 'vulcano_term_meta_field_edit_screen__after_' . $this->id, '</td>', $field );
			echo '</tr>';
		}
		echo '</table>';
	}
	...
	public function delete_fields( $term, $tt_id = null, $deleted_term = null ) {
		global $wpdb;
		$option = sprintf( 'vulcano_term_meta_%s', $term );
	...
	protected function process_fields( $args ) {
		...
		switch ( $type ) {
			...
			case 'image_plupload':
				...
				break;
			default:
				do_action( 'vulcano_user_meta_field_' . $this->id, $type, $id, $current, $options, $attrs );
				break;
		}
	}
	/**
	...
	protected function field_upload( $id, $current, $attrs ) {
		echo sprintf( '<input type="text" id="%1$s" name="%1$s" value="%2$s" class="regular-text"%4$s /> <input class="button vulcano-upload-button" type="button" value="%3$s" /><br />', $id, esc_url( $current ), __( 'Select file', 'vulcano' ), $this->build_field_attributes( $attrs ) );
	}
	/**
	 * Image field.
	 *
	...
	public function save( $term_id, $tt_id = null ) {
		...
		foreach ( $this->fields as $field ) {
			...
			$new  = apply_filters( 'vulcano_save_term_meta_' . $this->id, $_POST[ $name ], $name );
			$option = sprintf( 'vulcano_term_meta_%s_%s', $term_id, $name );
			if ( $new && $new != $old ) {
