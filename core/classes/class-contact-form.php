...
class Vulcano_Contact_Form extends Vulcano_Front_End_Form {
	...
	protected function build_mail_message( $submitted_data, $attachments ) {
		...
		$message = apply_filters( 'vulcano_contact_form_message_header_' . $this->id, '' );
		...
		$message .= apply_filters( 'vulcano_contact_form_message_footer_' . $this->id, '' );
	...
	protected function build_mail_subject( $submitted_data ) {
		if ( ! empty( $this->subject ) ) {
			...
			// Default subject.
			return sprintf(
				__( 'Message sent by the form %s in %s at %s', 'vulcano' ),
				$this->id,
	...
	protected function format_mail_headers( $submitted_data ) {
		...
		return apply_filters( 'vulcano_contact_form_mail_headers_' . $this->id, $headers );
	}
