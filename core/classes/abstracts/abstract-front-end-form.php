	...
	protected function display_success_message() {
		...
		if ( isset( $_GET['success'] ) && 1 == $_GET['success'] ) {
			...
			if ( ! empty( $this->success ) ) {
				...
				$html .= '<p>' . __( 'Form submitted successfully!', 'vulcano' ) . '</p>';
			}
			$html .= '</div>';
		}
			...
			foreach ( $this->fields as $fieldset ) {
				foreach ( $fieldset['fields'] as $field ) {
					...
					switch ( $type ) {
						case 'email':
							if ( ! is_email( $value ) ) {
								$this->set_errors( sprintf( __( '%s must be an email address valid.', 'vulcano' ), '<strong>' . $label . '</strong>' ) );
							}
							break;
						case 'file':
							if ( $files ) {
	...
	public function init() {
		...
		if ( ! empty( $submitted_data ) && isset( $submitted_data['vulcano_form_action'] ) && $this->id == $submitted_data['vulcano_form_action'] ) {
			...
			if ( $this->is_valid() ) {
				...
				add_filter( 'vulcano_front_end_form_messages_' . $this->id, array( $this, 'display_error_messages' ) );
			}
		}
	}
