 ...
 */
function vulcano_pagination( $mid = 2, $end = 1, $show = false, $query = null ) {
	...
	if ( ! isset( $_GET[ 'infinity' ] ) ) {
		...
		if ( $total_pages > 1 ) {
			...
			$arguments = apply_filters( 'vulcano_pagination_args', array(
					...
					'mid_size'  => $mid,
					'type'      => 'list',
					'prev_text' => __( '&laquo; Previous', 'vulcano' ),
					'next_text' => __( 'Next &raquo;', 'vulcano' ),
				)
			);
 ...
 * Add in single.php <?php vulcano_related_posts(); ?>
 ...
 */
function vulcano_related_posts( $display = 'category', $qty = 4, $title = '', $thumb = true, $post_type = 'post' ) {
	global $post;

	$show = false;
	$post_qty = (int) $qty;
	! empty( $title ) || $title = __( 'Related Posts', 'vulcano' );
		...
		if ( $related->have_posts() ) {
			...
			while ( $related->have_posts() ) {
				...
				if ( $thumb ) {
					...
					$image = apply_filters( 'vulcano_related_posts_thumbnail', get_the_post_thumbnail( get_the_ID(), 'thumbnail' ) );
 ...
 */
function vulcano_excerpt( $type = 'excerpt', $limit = 40 ) {
	$limit = (int) $limit;
...
function vulcano_thumbnail( $width, $height, $alt, $crop = true, $class = '', $upscale = false ) {
	...
	if ( $thumb ) {
		$image = vulcano_get_image_url( $thumb, $width, $height, $crop, $upscale );
		...
		return apply_filters( 'vulcano_thumbnail_html', $html );
	}
}
 ...
 *
 * Use:
 * add_action( 'the_post', 'vulcano_autoset_featured' );
 * add_action( 'save_post', 'vulcano_autoset_featured' );
 * add_action( 'draft_to_publish', 'vulcano_autoset_featured' );
 * add_action( 'new_to_publish', 'vulcano_autoset_featured' );
 ...
 * @return void
 */
function vulcano_autoset_featured() {
	global $post;
 ...
 */
function vulcano_debug( $variable ) {
